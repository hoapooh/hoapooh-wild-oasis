...
export async function getBookings({ filter, sortBy, page }) {
	// here if you use the ``, you can separate tlike cabins (*) cabins (id)
	// but if you use the '', you need it to stick next to the table name cabins(*) or cabins(id)
	let query = supabase.from('bookings').select(
		`id, created_at, startDate, endDate, numNights, numGuests, totalPrice, status, cabins (name), guests (fullName, email)`,
		// You use this when you don't want to query all of the data but only the result
		// This will be the second argument
		// View this count argument at line 27 in useBookings.js
		{ count: 'exact' }
	)

	// Here if you use many filter condition, you can loop through it to add more if necessary
